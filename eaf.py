...
class ColadaDeAcero:
    ...
    def cargar_analisis(self, archivo_csv):
        with open(archivo_csv) as f:
            ...
            for index, row in enumerate(rows):
                    if index == 0:                    
                        columns_dict = {}
                        for column_number, keys in enumerate(row):
                            columns_dict[keys] = column_number
                        continue
                    elements_dict.append([row[1], row[4], row[5]])
           
            self.carbono = elements_dict[0]
            self.silicio = elements_dict[1]
            self.manganeso = elements_dict[2]
            self.fosforo = elements_dict[3]
            self.azufre = elements_dict[4] 
            self.cromo = elements_dict[5] 
            self.molibdeno  = elements_dict[6]
            self.niobio = elements_dict[7]
            self.niquel = elements_dict[8]
            self.cobre = elements_dict[9] 
            self.titanio = elements_dict[10] 
            self.aluminio = elements_dict[11] 
            self.vanadio = elements_dict[12]  
